...
def transform_weather_data(daily_df, hourly_df):
    ...
    # ---------------- Decision Logic ----------------
    def decide(row):
        today_rain = row["rain_sum"]
        yesterday_rain = row["yesterday_rain"]
        next_6hr_rain = row["next_6hr_rain"]
        et0 = row["et0"]
        humidity = row["avg_humidity"]

        # 1. Hard SKIP rules (rainfall dominates)
        if today_rain >= 5 or yesterday_rain >= 5:
            return "SKIP", "Significant rainfall"
        if next_6hr_rain >= 2:
            return "SKIP", "Rain expected"

         # 2. ETₒ-driven WATER rules
        if et0 >= 5:
            return "WATER", "Dry Condition"
        if et0 >= 4 and humidity < 40:
            return "WATER", "Dry Condition"
        if et0 >= 3 and humidity < 50:
            return "WATER", "Dry Condition"

        # 3. Default SKIP (conditions not dry enough)
